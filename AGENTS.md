...
- `risk_days_bs`, `risk_days_prep_bs`, `iwp_from_lookback_data`, `residual_risk_rd`, `total_residual_risk_rd` — top-level estimation functions
  ...
  - **Both** `risk_days_bs` and `risk_days_prep_bs` accept `limits` (default `(-100, 500)`) — the integration domain (days) of the RDE integral, and the **single source of truth** for it: Python sends it to Go via the JSON bridge (`"limits"` → `RiskDaysInput.Limits [2]float64`), so both backends integrate the same domain. (Previously Go hardcoded `-100`/`500` in four places in `riskdays.go` and could not be driven from Python — duplicated and free to drift.) Validated identically on both paths: finite, `limits[0] < limits[1]` (NaN and ±Inf rejected). The default is a deliberately generous, numerically safe choice — the integrand is ≈0 well inside it — and **the app never sets it**; it exists as an escape hatch for exotic parameterisations (e.g. a PrEP `ser_max` beyond the 500-day upper limit, which the domain would otherwise silently truncate). Omitting `limits` from a direct Go-binary payload still defaults correctly, because `SetDefaults()` runs before `Validate()`.
  - `risk_days_prep_bs` also takes `a_dist_uniform` / `b_dist_uniform` (default `None`). The sinusoidal set-point oscillation params `a` (amplitude) and `b` (frequency) are **held fixed** at their scalar values unless a `(lo, hi)` tuple is given, in which case they are sampled `Uniform(lo, hi)` per bootstrap iteration. `offset` is never varied. `a` — and the upper bound of `a_dist_uniform` — must be `<= offset` (enforced in Python and Go `Validate()`), since `a > offset` would drive the plateau viral load negative; with `offset` fixed at 1 in the app, the UI simply caps the *a* input and the *a*-range slider at **1.0**, so that error is unreachable from the UI. Prior published analyses kept `a`/`b` fixed (a=0.7, b=0.6); the UI exposes this via a "Vary sinusoidal oscillation parameters (a, b)" checkbox (off by default; default ranges a∈(0.5,0.9), b∈(0.4,0.8)).
  - `risk_days_prep_bs`'s `offset` (default `1`, must be **> 0**) is the plateau's central level as a multiple of the set-point. **`tcrit` targets that central level** — `eclipse + doubling_time * log2((offset * set_point / χ) / C0)` — *not* the bare set-point: the plateau *begins* at `offset * set_point / χ` (the sinusoid starts at zero phase), so targeting the set-point made the modelled viral load **jump by exactly a factor of `offset` at `tcrit`** for any `offset != 1` — physically impossible, and UI-reachable at the time (0.05–2.0). Fixed on `fix-prep-tcrit-offset` in both backends (`_find_tcrit` / Go `FindTcrit`); **bit-for-bit no-op at the default `offset = 1`**, so no published result moves (verified against `main`). Consequently `offset` is now *exactly* a set-point multiplier — `(set_point, o, a)` ≡ `(set_point·o, 1, a/o)` — so **the UI no longer exposes it**: it added no expressive power while being a first-order lever on the answer (sweeping 0.5→2.0 moves the RDE ~5×, since the plateau sits near the pooled-NAT detection threshold and ~70% of the PrEP RDE accrues there). Vary `set_point` instead — clinical units, its own bootstrap range, and it drives `tcrit` correctly. The kwarg stays on the API for library callers and prior analyses.
  ...
  - **The four *k* input modes are mutually exclusive, and now enforced** (`core._validate_k_inputs`, mirrored in the Go `Validate()`): pass **exactly one** of `k_posterior_sample`, `k_gamma_shape`+`k_gamma_scale` (legacy Gamma), `k_invgamma_alpha`+(`k_invgamma_beta` **or** `k_invgamma_mode`), or the five `k_lnmix_*` parameters. A mode counts as *specified* if **any** of its parameters is non-`None`, so a partial spec (`k_gamma_shape` without `k_gamma_scale`) is an error too, as is giving both `k_invgamma_beta` and `k_invgamma_mode` (matching the public `sample_invgamma()`, which already raised on both). They were previously a **silent priority cascade** (posterior > gamma > invgamma > lnmix): passing two ran the higher-priority one with no warning, so a stale `k_posterior_sample` in a reused config dict quietly turned an "InvGamma sensitivity analysis" back into the posterior, and a partial spec fell through to whichever *other* mode was populated. Validated **pre-dispatch**, so both backends reject the same inputs in-process (not inside a worker or the Go subprocess). The app was never exposed — it passes explicit `None` for every unselected mode.
  - `total_residual_risk_rd`'s **joint** credible interval requires **every component to use the same k-distribution** (same mode *and* the same parameters). It receives only `(iwp_pe, iwp_bs, incidence, incidence_norm_sd)` per component, so it **cannot validate this for you**: mixing k-distributions makes the per-iteration `k` differ across components by construction, the shared-parameter alignment the joint CrI rests on collapses, and the summed-quantile interval silently becomes neither a joint interval nor a clean independence approximation. The app is safe (one k-distribution selector drives every component); the exposure is for **library** callers assembling components themselves. Related caveat: on the **Python** path the shared draws differ **by k-mode even at one fixed seed** — each k-distribution consumes a different amount of RNG entropy, shifting every subsequent draw — so a fixed seed does *not* reproduce the same shared parameters across k-modes. Only the **Go** backend guarantees alignment (the shared params are drawn *before* the baseline/PrEP branch, so PrEP-specific draws cannot perturb them).
...
- **`residualrisk/prep.py`** — PrEP-breakthrough model
  - Breakthrough viral dynamics (eclipse → growth → oscillating plateau; analytic `tcrit`, targeting the plateau's central level `offset * set_point / χ` so the trajectory is **continuous** at the crossover — see the `offset` bullet in the Public Python API section)
...
- **`residualrisk/_go.py`** — Go binary wrapper
  ...
  - Progress monitoring
  - `find_go_binary()` search order: `$RESIDUALRISK_GO_BINARY` env var → `<repo>/go/bin/riskdays_go` → the wheel-bundled `residualrisk/_bin/riskdays_go-<goos>-<goarch>` (built by `hatch_build.py`) → `~/.local/bin/riskdays_go` (sudo-free) → `$PATH`. **Every candidate is smoke-tested** with `_binary_runs()` (runs `--version`, requires exit 0; cached per path in `_binary_ok`, since `find_go_binary()` is called at load, at dispatch, and for the UI backend gate) — candidates used to be returned on *mere existence*, so a present-but-unrunnable binary (wrong architecture, corrupt, non-executable, missing shared library) was "found", `estimator.py`'s `used_go` gate (`find_go_binary() is not None`) reported Go acceleration as available, and only the first dispatch discovered otherwise, silently degrading to the 10–50× slower Python engine. The **bundled** binary keeps a stricter run-*and*-version-match check (`_binary_version_matches` against `__version__`): a mismatch there means a stale wheel, whereas a dev `go/bin/` build may legitimately lag. An **explicitly set** `$RESIDUALRISK_GO_BINARY` that doesn't run is `logging.warning`-ed rather than silently substituted.
...
- **Publishing** (`.forgejo/workflows/release.yml`, on Codeberg's **hosted** runner — no self-hosted
  ...
  `codeberg-tiny|small|medium` (1/2/4 CPU, 2/5/10-min caps). The cold six-target cross-compile
  measures ~20s on 2 cores, so with setup-go + uv + publish the job fits well inside small's 5-min
  cap (tiny's 2-min cap is too tight for the fixed setup overhead; `small` also schedules faster than
  the scarcer `medium` runners). (Self-hosting a runner is the alternative if the repo must stay
  private — set `runs-on` to its label.)
