...
def _vl_postbt_vec(t, eclipse, C0, doubling_time, set_point, a, b, offset):
    ...
    conc_attenuated = append(conc_attenuated, set_point * _sin_varied(t = newt, a = a, b = b, offset = offset))
...
def _prob_infectious_prep(t, 
    ...
    tmp, tcrit = _vl_postbt_vec(t = arange(0,265,0.1), eclipse = eclipse, C0 = C0, doubling_time = doubling_time, set_point = set_point, a = a, b = b, offset = offset)
    C = _vl_postbt(t = t, eclipse = eclipse, C0 = C0, doubling_time = doubling_time, set_point = set_point, a = a, b = b, offset = offset, tcrit = tcrit)
    n_copies = C * copies_per_virion * volume_transfused
    prob = _prob_infectious_copies(n_copies, k)
    return prob

def _prob_nondetection_serology_prep(t,
                               min,
                               max,
                               alpha,
                               beta,
...
def _prob_nondetection_prep(t,
    ...
    from numpy import arange
    tmp, tcrit = _vl_postbt_vec(t = arange(0,265,0.1), eclipse = eclipse, C0 = C0, doubling_time = doubling_time, set_point = set_point, a = a, b = b, offset = offset)
    Cv = _vl_postbt(t = t, eclipse = eclipse, C0 = C0, doubling_time = doubling_time, set_point = set_point, a = a, b = b, offset = offset, tcrit = tcrit)
    Cc = copies_per_virion * Cv
    if Cc == 0.0:
        return 1.0
    elif Cc > 0.0:
        p_pos_init = _prob_pos_init(Cc, doubling_time, pool_size, lod50, 
                                   lod95_lod50_ratio, z)
        p_neg_retest = _prob_neg_retest(Cc, doubling_time, pool_size, lod50, 
                                       lod95_lod50_ratio, retests, z)
        prob = 1 - p_pos_init * (1 - p_neg_retest)
        return prob

def _prob_infectious_nondetection_prep(t,
                                 eclipse,
                                 C0,
                                 doubling_time,
                                 set_point,
    ...
    product = _prob_infectious_prep(
                        ...
                        k = k) * _prob_nondetection_prep(t = t,
                                              ...
                                              retests = retests,
                                              z = z) * _prob_nondetection_serology_prep(t,
                                                                                  min = ser_min,
                                                                                  max = ser_max,
                                                                                  alpha = ser_alpha, 
                                                                                  beta = ser_beta
...
def _risk_days_prep(
    ...
    rd = quad(_prob_infectious_nondetection_prep, limits[0], limits[1], limit = 500,
              args=(eclipse, C0, doubling_time, set_point, a, b, offset,
                    volume_transfused, k, copies_per_virion, pool_size,
                    lod50, lod95_lod50_ratio, retests, ser_min, ser_max,
                    ser_alpha, ser_beta))[0]
    return rd
...
def risk_days_prep_bs(
    ...
    retests, 
    set_point = 336, # new
    set_point_dist_uniform = (19.1,2265), # new
    eclipse = 7.0, # new
    eclipse_dist_uniform = (4.0,10.0), # new
    a = 0.7, # new
    b = 0.6, # new
    offset = 1, # new
    ser_min = 28.7, # new
    ser_max = 250, # new
    ser_alpha = 50.49434, # new
    ser_beta = 1.15062, # new
    C0 = 0.00025, 
    copies_per_virion = 2, 
    alpha = 0.05,
    z = 1.6449,
    ...
    if k_posterior_sample is not None:
        ...
        raise ValueError("k_posterior_sample and k_gamma parameters must not both be 'None'.")
    doubling_times = stats.truncnorm.rvs(0, 
                                         np.inf, 
                                         doubling_time, 
                                         doubling_time_norm_sd, 
                                         n_bs)
    ...
    volumes_transfused = np.random.uniform(volume_transfused_range[0],
        ...
        for future in as_completed(futures):
            ...
            if progress is not None:
                ...
                if completed_count == 1 or new_percent > getattr(progress, '_last_percent', 0):
                    ...
                    progress.progress(progress_percentage, text=f"Progress: {new_percent}%")
