...
Companion to the base-model documentation above — document the PrEP breakthrough
model (`residualrisk/prep.py` / `go/riskdays/prep*.go`) for the same scientific
audience, reusing the analysis notebooks (`residualrisk_analysis/notebooks/`,
`rr_prep_v3.py`) and the ISBT 2025 materials wherever possible. Same conventions as
`theory.md` (GitHub-flavoured `$…$`/`$$…$$` KaTeX, GFM tables, figures generated by
`make_theory_figures.py`).

- [ ] **Decide structure** — extend `docs/theory.md` with PrEP `##` sections (the
      accordion picks them up automatically; renumber so References stays last) vs.
      a separate `docs/theory_prep.md` + second Documentation page. *Recommend
      extending `theory.md`* — one coherent document, one Documentation page.
- [ ] **Overview & scope** — PrEP breakthrough infection; oPrEP (oral) and iPrEP
      (injectable) as independent scenarios layered **additively** on the baseline
      window-period risk; cross-reference the Layer 1 / Layer 2 boundary and the
      one-product-per-run design (see Scope & validation / README).
- [ ] **Breakthrough viral dynamics** — the sinusoidal oscillating-plateau VL model
      (`_vl_postbt`, with analytic `tcrit` via `_find_tcrit`): eclipse phase, set
      point, `offset`, amplitude `a`, frequency `b`; the `max(0, Cv)` clamp; contrast
      with the base exponential-growth model; state the `a ≤ offset` constraint and
      why (negative plateau VL otherwise).
- [ ] **Seroconversion / serology cutoff** — Weibull seroconversion (oPrEP / iPrEP
      parameters); how the eclipse + serology cutoffs give the integrand **compact
      support** (this is what makes the GL-vs-quad choice a correctness issue —
      adaptive quad returns ~0).
- [ ] **Infectivity & drug effect** — the `drug_effect ∈ (0, 1]`
      transmissibility-reduction factor: in-integrand placement and why, optional
      `Uniform(lo, hi)` per-iteration variation; note the current constant-in-`t`
      placeholder (`_drug_effect` / `DrugEffectFactor`) and forward-reference the
      deferred PK/PD extension.
- [ ] **PrEP IWP integral & numerical methods** — the compact-support integral;
      Gauss-Legendre required (quad fails → ~0); analytic `tcrit`; Python≡Go parity;
      Go acceleration.
- [ ] **Uncertainty / bootstrap** — parameters varied (set point, eclipse uniform;
      optional `a` / `b` uniform; optional `drug_effect` uniform); the
      truncnorm-positivity fix applies here too; k input-distribution options are
      shared with the base model.
- [ ] **oPrEP vs iPrEP** — the two scenarios and their additive combination on the
      baseline; the structural caveat that the sinusoidal VL model best fits
      sub-optimal-adherence **oral** PrEP and less so **injectable** drug-washout
      (forward-reference the PK/PD deferred work).
- [ ] **Default parameters table & worked example** — production PrEP defaults; a
      worked PrEP RDE (≈3.09 at production serology) and an oPrEP + iPrEP
      residual-risk example consistent with the end-to-end validation (ISBT 2025).
- [ ] **Figures** — extend `make_theory_figures.py` with PrEP figures: breakthrough
      VL trajectory (eclipse → oscillating plateau, with/without `a`/`b` variation),
      PrEP IWP construction (compact support), and a `drug_effect` / k sensitivity
      panel.
- [ ] **References & version** — PrEP breakthrough-infection literature; the prior
      analysis (`rr_prep_v3`) and ISBT 2025; update the `theory.md` version footer to
      the release version when the PrEP documentation lands.
- [ ] **EG review pass (PrEP)** — verify equations, prose, citations, and worked
      numbers.
