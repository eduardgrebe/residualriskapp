...
- [x] **`core.py` — k input modes were a silent priority cascade** ✅ FIXED (`fix-k-mutual-exclusivity`) — `_sample_k` (and Go `RiskDaysBS`) dispatched on the *first* non-`None` k-parameter in a fixed order (posterior > gamma > invgamma > lnmix), so specifying two silently ran the higher-priority one, contradicting AGENTS.md's "mutually exclusive" claim. The failure was quiet and scientific: a library caller reusing a config dict that still carried `k_posterior_sample` while setting `k_invgamma_alpha`/`beta` got the **posterior**, so an "InvGamma sensitivity analysis" was silently the distribution they were trying to vary away from. A *partial* spec was equally silent — `k_gamma_shape` without `k_gamma_scale` fell straight through to whichever other mode was populated. New `core._validate_k_inputs()` requires **exactly one** fully-specified mode (a mode counts as specified if *any* of its parameters is non-`None`, which catches partial specs), and rejects `k_invgamma_beta` **and** `k_invgamma_mode` together — aligning `_sample_k` with the public `sample_invgamma()`, which already raised on both. Called **pre-dispatch** from `risk_days_bs`/`risk_days_prep_bs` (so the Go path fails identically and in-process, not in a worker or the subprocess) and from `_sample_k`; mirrored in Go `models.go` `Validate()` as the backstop for hand-written JSON payloads. The app was never exposed (it passes explicit `None` for every unselected mode). Tests: `tests/test_k_mode_exclusivity.py` (27 — all six mode pairs, partial specs, both backends, the Go binary) + 4 Go `TestValidate_*`. **Three existing tests pinned the old error wording and one — `test_posterior_takes_priority` — asserted the cascade itself; that one is replaced by `test_multiple_k_modes_raise`.**
...
- [x] **`prep.py` — analytic `tcrit` gave growth→plateau continuity only at `offset==1`** ✅ FIXED (`fix-prep-tcrit-offset`) — `_find_tcrit` solved for the time growth reaches the *bare set-point*, while the plateau branch **begins** at `offset * set_point / χ` (the sinusoid starts at zero phase, `sin(0)=0`). The two agreed only at `offset == 1`; at any other offset the modelled viral load **jumped by exactly a factor of `offset` at `tcrit`** — instantaneous and physically impossible — and `offset` was UI-reachable over 0.05–2.0. **Retargeted `tcrit` to the plateau's central level** in both backends (`_find_tcrit` / Go `FindTcrit`): `eclipse + doubling_time * log2((offset * set_point / χ) / C0)`. Continuity now holds for every `offset > 0` (`offset <= 0` rejected in Python and Go `Validate()` — the target level would be non-positive and `tcrit` `-inf`). **Bit-for-bit no-op at the default `offset = 1`** — verified by running the PrEP bootstrap at the app defaults on this branch and on `main` (identical PE *and* CrI to the last digit), so no published result moves. **Impact of the discontinuity itself was small** (≤1.1% on the RDE across the whole offset range; retargeting only shifts `tcrit` by `doubling_time * log2(offset)`, under a day, and leaves the plateau *level* untouched) — this was model hygiene, not a results defect. **`offset` removed from the UI**, which was the more consequential finding: after the retarget it is *exactly* a set-point multiplier — `(set_point, o, a)` ≡ `(set_point·o, 1, a/o)`, same `tcrit`, same plateau, same RDE — so it added zero expressive power while being a **first-order lever** on the answer (sweeping 0.5→2.0 moved the RDE from 4.75 to 0.89 days, ~5×, because the breakthrough plateau sits right at the pooled-NAT detection threshold and **~71% of the PrEP RDE accrues in the plateau**, not the growth phase). The set-point is the parameter to vary: clinical units, its own bootstrap range, and it drives `tcrit` correctly. The `offset` kwarg stays on the Python API (default 1) for library callers and for reproducing prior analyses. Amplitude `a` and its range slider are now capped at 1.0 in the UI, making `a > offset` unreachable there. Tests: `tests/test_prep_tcrit_offset.py` (continuity across offsets, closed form, no-op at 1, the reparameterisation identity, Python↔Go parity at `offset != 1`, validation on both paths + the Go binary) + 4 Go `TestFindTcrit_*`/`TestValidate_RejectsNonPositiveOffset`; two UI tests that pinned the old Offset widget were replaced by `TestOffsetNotExposed` (guarding against reintroducing the trap), and the `a > offset` error-path test was retargeted to an inverted seroconversion window so the ValueError→`st.sidebar.error` path stays covered. Docs: `docs/theory_prep.md` §3.2 + the parameter-variation table.
...
- [x] **`helpers.go` — `ModeRounded` tie-break was nondeterministic** ✅ FIXED (`fix-go-engine-cleanups`, commit `f651bc3`) — `count > maxCount` picked an arbitrary winner among tied values, and Go randomises map iteration order, so tied modes came out differently run to run — contradicting the doc comment's claim to match scipy's "smallest tied". Now `count > maxCount || (count == maxCount && v < mode)`, i.e. deterministically the smallest tied value. Unused by the engine today (the UI reports the KDE mode), so no result changes.
- [x] **`kde.go` — Go KDE `cap` pre-sampling drew *with* replacement** ✅ FIXED (`fix-go-engine-cleanups`, commit `f651bc3`) — Python's `cap` subsamples *without* replacement, so the two backends' KDE modes could differ for the same capped posterior. Extracted `sampleWithoutReplacement()` (a `rng.Perm` prefix) to match. Unreachable from the API (`mode_kde_go` passes `cap=None`), hence the low priority — but the two implementations no longer disagree.
...
- [x] `README.md:170` ✅ FIXED (`fix-kde-mode-perf`) — replaced with a complete, runnable JSON example (k-InvGamma + all required fields; verified through the binary).
...
- [x] `core.py:289` ✅ FIXED (`fix-kde-mode-perf`) — docstring `n_grid` default corrected to `5_000`, with a note on the pure-Python (5 000) vs Go (100 000) grid split.
- [x] `_go.py:93` **+ KDE-mode grid standardisation** ✅ FIXED (`fix-kde-mode-perf`) — removed the false "matches the Go auto-default" claim. **Standardised the KDE-mode grid**: the Go path (`mode_kde_go` default, the Go binary's mode PE `riskdays.go:188/387`, and the app's load-time k-modes) now uses **100 000** (FFT, fast, ≡ 1M to ~4 sig figs); the pure-Python fallback (`_kde_mode_log` PE branches in `core.py`/`prep.py` + `mode_kde`) uses **grid 5 000, cap=None** — the grid (not the cap) is where the speed comes from, and keeping the full sample (`cap=None`) makes the fallback track the Go result to **<0.1%** even on a bimodal 1M stress sample (a parity test enforces this; capping to 50 000 had drifted to ~0.7–0.8% on large samples). Cost: `cap=None` is O(n·5 000), so the fallback is ~1 s at the default n_bs=100k, ~10 s only at the n_bs=1M extreme (rare, Go-unavailable). The app shows a warning when Go is unavailable, and the lnmix k-PE respects the backend choice (Go→Go, Python→pure-Python). Fixes a latent pure-Python slowdown: the b2 mode-default at `n_grid=1M, cap=None` was ~3.6 min at n_bs=100k **on the Python backend only** — the Go/app path was always fast via FFT (my earlier "3.6 min in the app" was wrong).
- [x] `random.go:43` ✅ FIXED (`fix-kde-mode-perf`) — comment corrected to `truncnorm.rvs(a=-mean/sd, …)`, noting scipy's `a`/`b` are standardised (SDs from `loc`), so `a=0` truncates at the mean, not 0.
...
- [x] `[test]` **`test_prep_bootstrap.py`** ✅ FIXED (`test-coverage-prep-recompute-and-golden`) — added `_assert_prep_rows_recompute_iwp`: every `sim_df` row's stored parameters must reproduce that row's `iwp` via `_risk_days_prep`. This is the only check that catches a param↔iwp **row misalignment** (the `core.py:640` bug class) — the pre-existing checks cannot, since a misaligned frame still has every column and its `iwp` column still equals `rdests`. Run on **both** backends (Go variant is sandbox-safe: subprocess, not `ProcessPoolExecutor`), including a custom-`limits` case proving the recompute reads each row's *recorded* domain. `test_primary_parameters` asserted `rd_pe > 0` **twice** (the second a copy-paste; the "plausible range" its comment promised was never checked) — now asserts the PE **equals** the recomputed single integration, which is exact, since the plug-in PE is deterministic. (A CrI/range bound would have been *wrong*: the plug-in PE can legitimately sit in the far right tail — see "PrEP RDE point-estimate default → mode".)
- [x] **Go `sim_df` did not record `limits`** ✅ FIXED (same branch; surfaced by the recompute work) — the Go frame carried every other parameter (`z`, `drug_effect`, `ser_*`, …) but not the integration domain, while the Python frame always has. Now that `limits` is caller-settable, a Go-path `sim_df` was silently failing to record the domain it used — a real gap for downstream sensitivity analysis, and a backend schema divergence. Both Go builds (baseline + PrEP) now emit `limits`, with dtype parity (`List(Int64)` for the default).
...
- [x] **Golden regression test** ✅ ADDED (`tests/test_golden_bootstrap.py`, branch `test-coverage-prep-recompute-and-golden`) — pins the **bootstrap headline** PE + CrI at a fixed seed. (The pre-existing goldens pin `_risk_days`, the *single integration*; nothing pinned the bootstrap's output, so a change to the sampling, aggregation, or RNG plumbing could pass every other test.) The two backends draw from **different RNG streams** (numpy vs Go), so a fixed seed gives different realisations of the same distributions — there is no shared literal, and each backend is pinned separately. A third test asserts the two agree within Monte-Carlo error, which is the only thing that catches one backend's *distribution* drifting (a per-backend golden alone would just be regenerated). Verified thread-count independent (the bootstrap pre-draws from the seed and fills by index), so the pins reproduce regardless of worker count.
- [x] **Load-time Go-binary smoke test** ✅ ADDED (`batch3-go-smoke-test-docs-ci`) — **4 of the 5 `find_go_binary()` candidates were returned on mere existence** (`$RESIDUALRISK_GO_BINARY`, `go/bin/`, `~/.local/bin/`, `PATH`); only the wheel-bundled binary was ever executed. A present-but-unrunnable binary (wrong architecture, corrupt, non-executable, missing shared library) was therefore "found", `estimator.py`'s `used_go` gate (`find_go_binary() is not None`) reported Go acceleration as **available**, and only the first dispatch discovered otherwise — silently degrading to the 10–50× slower Python engine. Every candidate is now gated on `_binary_runs(path)` (runs `--version`, requires exit 0; cached per path in `_binary_ok`, since `find_go_binary()` is called at load, at dispatch, and for the UI gate). The bundled binary keeps its **stricter** run-*and*-version-match check (a mismatch there means a stale wheel, whereas a dev `go/bin/` build may legitimately lag). An **explicitly set** `$RESIDUALRISK_GO_BINARY` that doesn't run now `logging.warning`s ("exists but does not run") rather than being silently substituted. Tests: 7 new in `tests/test_bundled_binary.py` (runs/missing/non-executable/exits-nonzero, caching, broken-override skip + log, `None` when nothing runs).
- [x] **Joint-CrI k-distribution requirement documented** ✅ DONE (`batch3-go-smoke-test-docs-ci`) — `total_residual_risk_rd` receives only `(iwp_pe, iwp_bs, incidence, incidence_norm_sd)` per component, so it **cannot validate** that all components used the same k-distribution; mixing them makes the per-iteration `k` differ by construction, the shared-parameter alignment collapses, and the summed-quantile interval silently stops being a valid joint CrI. Documented in the `total_residual_risk_rd` docstring (`core.py`) + `AGENTS.md`, together with the Python-path corollary: the shared draws differ **by k-mode even at one fixed seed** (each k-distribution consumes a different amount of RNG entropy, shifting every subsequent draw). The **app** is safe — one k-distribution selector drives every component — so the exposure is to *library* callers assembling components themselves; hence document, not enforce.
- [x] **`RiskDaysInput.LimitMin/LimitMax`** ✅ FIXED (`feat-expose-integration-limits`) — **the premise was wrong: the fields were never dead.** They live on the *internal* `RiskDaysInnerParams` (not `RiskDaysInput`) and *are* the Gauss-Legendre domain used by `integration.go` / `prep_integration.go`; `riskdays.go` merely hardcoded them to `-100`/`500` in four places while Python kept its own `_risk_days(limits=(-100, 500))` default — duplicated, undrivable from the caller, and free to drift. Fixed by **exposing `limits`** as a public parameter on `risk_days_bs` **and** `risk_days_prep_bs` (default `(-100, 500)`), wired through the JSON bridge to a new `RiskDaysInput.Limits [2]float64` and validated identically on both paths (finite, `limits[0] < limits[1]`; NaN/±Inf rejected). Python is now the single source of truth, and the knob is a genuine escape hatch — a PrEP `ser_max` beyond 500 d was previously truncated silently with no way to widen the domain. Omitting `limits` from a direct binary payload still defaults (`SetDefaults()` runs before `Validate()`), so `scripts/smoke_bundled_binary.py` and the README JSON examples are unaffected. Tests: `tests/test_integration_limits.py` (29) + Go `TestSetDefaults_FillsLimitsWhenUnset` / `TestSetDefaults_PreservesExplicitLimits` / `TestValidate_RejectsDegenerateLimits`.
- [x] **`core.py` — the "primary parameters" PE silently dropped `z`** ✅ FIXED (`feat-expose-integration-limits`; found en route) — the baseline point-estimate call passed only 9 positional args to `_risk_days`, so it used that function's *own* defaults (`z=1.6449`, `limits=(-100, 500)`) while the bootstrap used the caller's values. A custom `z` therefore produced a PE inconsistent with its own credible interval **and** with the Go backend, which does honour `z`. Baseline-only — `prep.py`'s PE already passed `z`. Same class as the fixed `prep.py:311` `z` omission. Regression test: `TestPointEstimateHonoursZ` (multiprocessing-marked).
...
- [x] **`docker build` check on PRs / `main`** ✅ ADDED (`.github/workflows/docker-build-check.yml`,
      branch `batch3-go-smoke-test-docs-ci`) — native single-arch (`linux/amd64`), `push: false`, gha
      cache, on `pull_request` + `main` pushes. Catches Dockerfile / `uv sync` breaks at the commit
      that causes them without publishing an image; the multi-arch build and the registry push stay
      in the tag job. Ubuntu runners are plentiful, so unlike the bundled-binary smoke test (macOS,
      concurrency-limited → tags-only) this can run per-commit. **Stays on GitHub even under the
      Codeberg-primary plan** (Deferred → "Migrate primary hosting to Codeberg"): Codeberg's hosted
      runners have no Docker daemon, so a `docker build` cannot run there.
